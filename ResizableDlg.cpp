...
{
	m_hWnd = hWnd; m_rect = rect; m_anchor = anchor; m_dxMove = dxMove; m_dyMove = dyMove; m_dxResize = dxResize; m_dyResize = dyResize; m_isTransparent = isTransparent;
}

CResizableDlg::CResizableDlg(UINT nIDTemplate, int dlgWidthMin, int dlgHeightMin, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent), m_hGripper(NULL), m_sizeMinimum(dlgWidthMin, dlgHeightMin)
{
}

CResizableDlg::~CResizableDlg()
{
	for (int i = 0; i < m_controls.GetSize(); i++) delete m_controls.GetAt(i); m_controls.RemoveAll();
}
...
void CResizableDlg::loadDlgPlacement()
{
	...
	{
		if (nBytes == sizeof(WINDOWPLACEMENT)) SetWindowPlacement((WINDOWPLACEMENT*)pBytes);
		delete[] pBytes;
	}
}
...
{
	int i, dx, dy; UINT uFlags; CRect rectNew; HDWP hDefer = NULL;

	CDialogEx::OnSize(nType, cx, cy);

	if (hDefer = BeginDeferWindowPos((int)m_controls.GetSize()))
	{
		dx = cx - m_sizeInitial.cx; dy = cy - m_sizeInitial.cy;
		for (i = 0; i < m_controls.GetSize() && hDefer; i++)
		{
			rectNew = m_controls.GetAt(i)->m_rect; // Rectangle of control at initial window.
			if (m_controls.GetAt(i)->m_anchor == tControl::anchor::TOP_LEFT)
			{
				...
				rectNew.right += (int)(dx * m_controls.GetAt(i)->m_dxResize); rectNew.bottom += (int)(dy * m_controls.GetAt(i)->m_dyResize);
			}
			else if (m_controls.GetAt(i)->m_anchor == tControl::anchor::BOTTOM_LEFT)
			{
				...
				rectNew.right += (int)(dx * m_controls.GetAt(i)->m_dxResize); rectNew.top -= (int)(dy * m_controls.GetAt(i)->m_dyResize);
			}
			else if (m_controls.GetAt(i)->m_anchor == tControl::anchor::TOP_RIGHT)
			{
				...
				rectNew.left -= (int)(dx * m_controls.GetAt(i)->m_dxResize); rectNew.bottom += (int)(dy * m_controls.GetAt(i)->m_dyResize);
			}
			else // BOTTOM_RIGHT
			{
				rectNew.OffsetRect((int)(dx * (1 - m_controls.GetAt(i)->m_dxMove)), (int)(dy * (1 - m_controls.GetAt(i)->m_dyMove)));
				rectNew.left -= (int)(dx * m_controls.GetAt(i)->m_dxResize); rectNew.top -= (int)(dy * m_controls.GetAt(i)->m_dyResize);
			}
	...
	if (m_hGripper) ::ShowWindow(m_hGripper, (nType == SIZE_MAXIMIZED) ? SW_HIDE : SW_SHOW);
}

void CResizableDlg::OnDestroy()
{
	saveDlgPlacement();
	if (m_hGripper) { ::DestroyWindow(m_hGripper); m_hGripper = NULL; }
	for (int i = 0; i < m_controls.GetSize(); i++) delete m_controls.GetAt(i); m_controls.RemoveAll();
	CDialogEx::OnDestroy();
}